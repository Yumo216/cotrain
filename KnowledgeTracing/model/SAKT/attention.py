...
class Encoder(nn.Module):
    ...
    def __init__(self, h, length, hidden, dropout):
        ...
        self.sublayer = clones(SublayerConnection(length, hidden, dropout), 2)

    def forward(self, x, y, x_co, mask=None):
        "Follow Figure 1 (left) for connections."
        y = self.sublayer[0](y, lambda y: self.multi_headed_attention(x, y, y, x_co, mask))
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, x_co, mask=None):
        if mask is not None:
        ...
        x = self.attention(query, key, value,x_co,causality=True, mask=mask,
    ...
    def attention(self, query, key, value,x_co, causality=True, mask=None, dropout=None):
...
class SublayerConnection(nn.Module):
    ...
    """
    def __init__(self, length, hidden, dropout):
        super(SublayerConnection, self).__init__()
        self.norm = nn.LayerNorm(normalized_shape = [length, hidden])  # 用一个维度
        self.dropout = nn.Dropout(dropout)
